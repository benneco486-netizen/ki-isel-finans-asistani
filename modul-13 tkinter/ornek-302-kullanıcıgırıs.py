...
window = Tk()    # Tk sınıfını 'window'a atadık.
window.title("Kullanıcı Giriş Ekranı")    # Pencere Başlığı
window.geometry("390x220") # Pencere Ölçüsü
window.iconbitmap("einstein.ico")  #Pencere ikonunu ekledik
...
def giris():
    ...
    if (E1.get() == str("admin")) and (E2.get() == str("123456")):
